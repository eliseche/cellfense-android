...
public class Explosion {
	...
	private static float gravity;					// the gravity of the explosion (+ upward, - down)
	...
	public Explosion(int particleNr, int x, int y, float dx, float dy) {
		this.state = STATE_ALIVE;
		this.particles = new Particle[particleNr];
	 	for (int i = 0; i < this.particles.length; i++) {
			Particle p = new Particle(x, y, dx, dy);
			this.particles[i] = p;
		}
	...
	public static void setGravity(float pgravity) {
		gravity = pgravity;
	}
